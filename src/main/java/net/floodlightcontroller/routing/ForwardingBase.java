...
public abstract class ForwardingBase 
    ...
    //add by wunyuan
        ...
        //match.setNetworkProtocol((byte)1);
        ...
        for (int indx = switchPortList.size()-1; indx > 0; indx -= 2) {
            ...
            try {
                ...
                if(FlowData.CheckFlowFromMap(cntx))
                {
                	if(!FlowData.flowMap.containsValue(route))
                	{
                		FlowData.removeFlowFromMap(cntx);
                		FlowData.addFlowToMap(cntx, route);
                	}
                }
                else {
            		FlowData.addFlowToMap(cntx, route);
